...
import { postOffer } from '../../utils/apiUrl'
...
const ListOffer = () => {
    ...
    const uploadPicture = async (e) => {
        ...
        // console.log(formData, file, url)
        try {
            const response = await fetch(postOffer, {
                method: "POST",
                body: formData,
            })
            ...
            if (res.success) {
                setLoading(false)
                toast.success(response.me, toastOptions)
                setTimeout(() => {
                    setUrl("")
                    console.log(url)
                }, 2000)
        ...
        <div className='w-full'>
            ...
            <div className='flex absolute w-full'>
                ...
                <div className='w-full min-h-screen  absolute right-0 flex flex-col items-center justify-center gap-8 md:w-4/5'>
                    <h1 className='text-4xl font-semibold'> List new offer </h1>
                    <input onChange={(e) => {
                        setUrl(e.target.value)
                    }} type="text" placeholder="Offer URL" className='w-4/5 h-10 rounded-md border border-gray-400 outline-gray-400 hover:outline-rose-500 pl-4 text-xl' />
